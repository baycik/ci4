...
class User extends Catalog {
    ...
    private function userInformBySms($user_data,$new_user_pass){
        ...
        $initial_user_level=session('user_level');
        session('user_level',1);//elevate user level for accessing sms credentials
        ...
        session('user_level',$initial_user_level);
	...
	$session->set('user_level', $user_data->user_level);
	$session->set('user_level_name', $this->Hub->level_names[$user_data->user_level]);
	$session->set('user_login', $user_data->user_login);
	$session->set('user_sign', $user_data->user_sign);
	$session->set('user_position', $user_data->user_position);
	$session->set('user_assigned_stat',$user_data->user_assigned_stat);
	$session->set('user_assigned_path',$user_data->user_assigned_path);
        $session->set('user',$user_data);
	$session->set('modules_allowed',$this->getModuleList());
	...
	$this->log(session('user_login')." signed out");
        $_SESSION = array();
	return true;
    }
    public $getUserData=[];
	...
	return [
	    ...
	    'acomp'=>session('acomp'),
	    'pcomp'=>session('pcomp'),
	    'module_list'=>session('modules_allowed')
	];
    }
    private function getModuleList(){
        $issConfig = config('IssConfig');
	$alowed=array();
	foreach( $issConfig->modules as $mod ){
	    if( session('user_level')>=$mod['level'] ){// && strpos(BAY_ACTIVE_MODULES, "/{$mod->name}/")!==false 
		$alowed[$mod['name']]=$mod;
	    }
	}
	return $alowed;
	...
	$user_id = session('user_id');
        $where = (session('user_level') < 4) ? "WHERE user_id='$user_id'" : "";
        $sql="SELECT
	...
	if( $current_level>=1 && session('user_id')==$user_id || $current_level>=4){
